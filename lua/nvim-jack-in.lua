...
local function jack_in(execution_string)
  ...
  local cwd = tostring((type(M.config.cwd) == "function" and M.config.cwd() or M.config.cwd) or vim.uv.cwd() or "")
  if M.config.force_powershell == true then
    vim.cmd(":term powershell -Command \"Set-Location -Path '" .. cwd .. "'; " .. execution_string .. '"')
  else
    vim.cmd(":term " .. "cd " .. vim.fn.shellescape(cwd) .. " && " .. execution_string)
  end
